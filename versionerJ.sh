...
### Checkup ###
	...
	for item in ${files[*]}; do
		...
		niddle=${elmt[2]}
 		...
 		if [ ! $selector ]; then	
 			[ "$(cat "$filename" | sed /$niddle/!d)" ] || errExit "Version tag not found in '$filename'"	
 		else		
 			[ "$(cat "$filename" | sed -e /$selector/!d -e /$niddle/!d)" ] || errExit "Version tag not found in '$filename'"
 		fi
	done
...
# ### Main ###
	...
	for item in ${files[*]}; do
		...
		cat "$filename" | sed "s/\(${selector}.*${niddle}[ ]\{0,10\}['\"]\)\(.*\)\(['\"]\)/\1${1}\3/" >"${filename}_tmp"
